...
- The trust anchor is swappable but never removable. `--bridge-ca-file` (and the NixOS module's `bridge.caFile`) replaces the vendored `root-bridge` CA with another one; the CN assertion still runs on top of it. This exists for a Bridge behind a certificate the shipped CA cannot verify — and for the issue #14 integration test, whose fake Bridge presents a leaf of exactly the real shape under a CA it mints itself, so the whole verification path runs for real rather than being switched off. There is no flag that turns the CN check or `CERT_REQUIRED` off.
